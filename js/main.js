...
$(document).ready(function() {
  ...
  $('.login-page .setting-menu .setting-head > .icon-wrong').click(function() {
    $('.setting-menu').animate({
      left: "-500px"
    }, 300)
    $('.main-content').animate({
    ...
    $('.setting-pull').fadeIn();
    $('.no-action.setting-screen p').css({
      "margin-left": "0px"
    })
  })
  ...
  //    if ($(this).attr('class').indexOf('always-active') > 0) {
  //        return false;
  //    }
  ...
  $('.multi-select .dropdown-item').click(function() {
    ...
    if ($this.attr('class') === "dropdown-item active") {
      langArr.push(selectedLang);
      $('.selected-item').text(langArr.toString());
      if (selectedLang.toLowerCase() == "english") {
        $('.lan-list .addonInput').append(`<div class="sel-lang-block ${selectedLang}" > <div class="language-title">${selectedLang}</div> <div class="bannerInput-group"> <span class="banner-label">title</span> <input type="text" class="login-fields login-title" onkeyup="changeBannerText(this)" placeholder="title"> </div> <div class="bannerInput-group"> <span class="banner-label">subtitle</span> <input type="text" class="login-fields login-subtitle" onkeyup="changeBannerText(this)" placeholder="subtitle"> </div> <div class="bannerInput-group"> <span class="banner-label">link text</span> <input type="text" class="login-fields login-link-input" onkeyup="changeBannerText(this)" placeholder="link text"> </div> <div class="bannerInput-group"> <span class="banner-label">link url</span> <input type="text" onblur="ValidURL(this.value)" onkeyup="changeBannerText(this)" class="login-fields login-link-url" placeholder="link url"> </div> <div class="db-error-mgs"> <span>*Please enter the required fields</span> </div> <div class="db-btn input-submit sub-btn"> <span>Click to Preview</span> </div> </div>`);
      } else if (selectedLang.toLowerCase() == "arabic") {
        if ($('.Arabic').length < 1) {
          $('.lan-list .addonInput').append(`<div class="sel-lang-block ${selectedLang}"><div class="language-title">${selectedLang}</div> <div class="bannerInput-group"><span class="banner-label">title</span><input type="text" class="login-fields login-title" id="loginTitleArabic" onkeyup="arabicValue(loginTitleArabic)" dir="rtl" placeholder="title"></div><div class="bannerInput-group"><span class="banner-label">subtitle</span><input type="text" class="login-fields login-subtitle" id="loginSubTitleArabic" onkeyup="arabicValue(loginSubTitleArabic)" dir="rtl" placeholder="subtitle"></div><div class="bannerInput-group"><span class="banner-label">link text</span><input type="text"  class="login-fields login-link-input" id="loginLinkTextArabic" onkeyup="arabicValue(loginLinkTextArabic)" dir="rtl" placeholder="link text"></div><div class="bannerInput-group"><span class="banner-label">link url</span><input type="text" class="login-fields login-link-url" id="loginLinkUrlArabic" onblur="ValidURL(this.value)" placeholder="link url"></div><div class="db-error-mgs"> <span>*Please enter the required fields</span> </div> <div class="db-btn save-arabic sub-btn" id="save-arabic"><span>Click to Preview</span> </div> </div>`);
        }
      }
    } else {
      langArr.splice(($.inArray(selectedLang, langArr)), 1);

      $('div').remove(`.${selectedLang}`);
    }
    $('.selected-item').text(langArr.toString());
    console.log(langArr);
  });
  ...
  $('.login-setup .theme-item').click(function() {
    ...
    $('.login-page').attr('class', 'main-content login-page setting-page screen-' + logChild);
    ...
    $('#txtPassword').css('-webkit-box-shadow', '');

    //if (parseInt(Childno + 1) == 3) {
    //    //$('.login-footer a,.login-content-block > span, .login-footer .down-links a > span, .screen-3 label,.screen-3 .pass-eye, .down-links a .icon').css("color", primclr);
    //    $('#txtPassword').css('-webkit-box-shadow', '0 0 0 100px #ffffff00 inset');
    //}
    if ($('.login-page').attr('class').indexOf('screen-3') > 0) {
      $('.login-page').addClass('screen-3-login');
      $('.login-footer').addClass('screen-3-login');
      $('.login-content-block span').css('color', $('.re-me').css('color'));
    } else {
      $('.login-page').removeClass('screen-3-login');
      $('.login-footer').removeClass('screen-3-login');

      $('.login-content-block span').css('color', $('span.loginTitle').css('color'));
      //$('#btnLogin').css('color', $('.re-me').css('color'));

      //$('.login-head').css({ "background-color": "#ffffff" });
      //$('.login-head .right-block .imp-link a,'
      //+ '.login-head .right-block .imp-link a .icon,'
      //+ '.lang-sel .dropdown-block .selected-item').css({ "color": "#555" });
      //$('.dropdown-block .sel-down-arrow').css({ "border-top-color": "#555" });

      $('.login-head').css({
        "background-color": $('.hdBc-var .pickr-child').css("background-color")
      });
      $('.login-head .right-block .imp-link a,' +
        '.login-head .right-block .imp-link a .icon,' +
        '.lang-sel .dropdown-block .selected-item').css({
        "color": $('.hdTxt-var .pickr-child').css("background-color")
      });
      $('.dropdown-block .sel-down-arrow').css({
        "border-top-color": $('.hdTxt-var .pickr-child').css("background-color")
      });
  ...
  $(document).on('click', '.input-submit', function() {
    ...
    $('.loginTitle').css('text-align', 'left');
    $('.loginSubtitle').css('text-align', 'left');
    $('.login-link').parent().css('text-align', 'left');
  });
  $(document).on('click', '.save-arabic', function() {
    ...
    if ((loginTitleA.length) && (loginSubtitleA.length) && (loginLinkTxtA.length) && (loginLinkUrlA.length) > 0) {
      ...
      $('.login-link').attr('href', "https://" + loginLinkUrlA);
      $('.login-inputs input').css({
        "border": "0px solid"
      });
      $('.db-error-mgs').slideUp();
    ...
    $('.loginTitle').css('text-align', 'right');
    $('.loginSubtitle').css('text-align', 'right');
    $('.login-link').parent().css('text-align', 'right');
  });
    ...
    $.ajax({
      ...
      success: function(response) {
        if (response.DO != null) {
          ...
          alert("Unable to upload logo Image.");
          return false;
        }
      }
    });
    ...
    $.ajax({
      ...
      success: function(response) {
        if (response.DO != null) {
          ...
          alert("Unable to upload Background Image.");
          return false;
        }
      }
    });
  ...
  $('.pre-btn,.btn-yes,.history-btn > .flat-btn:first-child').click(function() {
    ...
    $('.setting-pull').fadeIn();
    $('.setting-menu .setting-pull').find('.icon').removeClass('icon-back');
  ...
  $('.punch-opt .db-toggle-btn').click(function() {
    if ($('.punch-opt .db-toggle-btn').attr('class').indexOf('disabled') >= 1) {
      return false;
    }
  ...
  $('.cc-opt .db-toggle-btn').click(function() {
    if ($('.cc-opt .db-toggle-btn').attr('class').indexOf('disabled') >= 1) {
      return false;
    }
  ...
  $('.titTxt-var .color-block').ColorPicker({
    ...
    onChange: function(hsb, hex, rgb) {
      ...
      $('.dashboard-page h4,.loginSubtitle,.login-link,.loginTitle').css("color", '#' + hex);
      $('.login-footer').css('color', '#' + hex);
      $('.login-footer span').css('color', '#' + hex);
    },
  ...
  $('.txt-var .color-block').ColorPicker({
    ...
    onChange: function(hsb, hex, rgb) {
      ...
      //    $('.login-footer a,.login-content-block > span,.icon-lock,.login-txt').css("color", '#' + hex);
      //}
      //else {
      //    $('.punch .db-btn,.login-body .login-block .login-title-block span.icon-lock,.login-body .login-block .login-title-block span.login-txt,input:focus + label, input:valid + label,.login-input-list .re-me.active,lable,.logincolor,.screen-3 .login-input-list a,.pass-eye,.screen-3 .login-footer a,.login-input-list .re-me,.login-input-list a,.login-body-block label').css("color", '#' + hex);
      //    $('.login-body-block input').css('border-color', '#' + hex);
      //}
      //$('.login-footer a,.login-content-block > span, .login-footer .down-links a > span, .screen-3 label,.screen-3 .pass-eye, .down-links a .icon').css("color", '#' + hex);
      ...
      if ($('.login-page').attr('class').indexOf('screen-3') < 0) {
        $('.login-body .login-block .login-title-block .login-txt,' +
          '.login-body .login-block .login-title-block span.icon-lock,' +
          '.login-body .login-block .login-body-block .pass-eye,' +
          '.login-body .login-block .login-body-block .punch .db-btn,' +
          '.login-body .login-block .login-body-block .re-me,' +
          '.login-body .login-block .login-body-block a,' +
          '.login-body .login-block .login-body-block label').css('color', '#' + hex);
        $('.login-body .login-block .login-body-block .punch .db-btn,' +
          '.login-body .login-block .login-body-block input').css("border-color", '#' + hex);
        $('.login-body .login-block .login-body-block .sub-btn').css("background-color", '#' + hex);
        $('.login-footer').css("background-color", '#' + hex);
      } else {
        $('.login-body .login-block .login-title-block .login-txt,' +
          '.login-body .login-block .login-title-block span.icon-lock,' +
          '.login-body .login-block .login-body-block .pass-eye,' +
...
function SaveThemeSetup() {
  ...
  if ($('div.main-content')[0].classList[2] == undefined) {
    objParam.LoginPageName = "4";
  } else {
    objParam.LoginPageName = $('div.main-content')[0].classList[3].replace('screen-', '').replace('Screen-', '');
  }
  ...
  objParam.Title = $('.login-title').val(); //$('.loginTitle').text();
  objParam.SubTitle = $('.login-subtitle').val(); //$('.loginSubtitle').text();
  objParam.LinkText = $('.login-link-input').val(); //$('.login-link').text();
  ...
  if ($('.language-select .selected-item').text().split(' ').join('') == "" || $('.language-select .selected-item').text().split(' ').join('') == undefined) {
    objParam.LanguageID = "English";
  } else {
  ...
  objParam.colourBackground = $('body').css("background-color");
  objParam.colourHeaderBackground = $('.hdBc-var .color-block .pickr-child').css('background-color'); //$('.db-head').css("background-color");
  objParam.colourHeaderText = $('.hdTxt-var .color-block .pickr-child').css('background-color'); //$('.login-head .right-block .imp-link a').css("color");
  objParam.colourTitleText = $('.titTxt-var .color-block .pickr-child').css('background-color'); //$('.dashboard-page h4,.loginTitle').css("color");
  objParam.colourText = $('.txt-var .pickr-child').css('background-color'); //$('.txt-var .loginSubtitle,.login-link').css('color');
  ...
  if (data.Code == "1") {
    if (data != null) {
      ...
      if (window.location.pathname.split('/').slice(-1)[0].toLowerCase() == "zing.aspx") {
        ...
        $('.login-page').attr('class', 'main-content login-page setting-page screen-' + data.LoginPageName);
      }
      ...
      if (data.LoginPageName == "3" || data.LoginPageName == "2") {
        $('.addonInput').css("display", "none");
      }
      ...
      $('.login-link').attr('href', 'https://' + data.InputFieldLinkUrl);
      var language = "";
      var isLan = false;
      ...
      if (data.ThemeHistory != undefined) {
      ...
      if (data.IsCompanyCode.toLowerCase() == "true") {
        ...
        $('.screen-3 .com-code').css('display', 'flex');
        $('.com-code').css('display', 'flex');
      } else {
      ...
      if ($('.login-page').attr('class').indexOf('screen-3') > 1) {
        $('.punch .db-btn,.login-body .login-block .login-title-block span.icon-lock,.login-body .login-block .login-title-block span.login-txt,input:focus + label, input:valid + label,.login-input-list .re-me.active,label,.screen-3 .login-input-list a,.screen-3 .pass-eye,.screen-3 .login-footer a,.login-content-block > span,.login-input-list .re-me,.login-input-list a').css("color", data.colourText); //rgb2hex($('.txt-var .color-block .pickr-child').css('background-color'))
        $('.screen-3 .login-body-block input').css('border-color', data.colourText);
      } else {
        $('.punch .db-btn,.login-body .login-block .login-title-block span.icon-lock,.login-body .login-block .login-title-block span.login-txt,input:focus + label, input:valid + label,.login-body-block label.login-input-list .re-me.active,label,.screen-3 .login-input-list a,.screen-3 .pass-eye,.screen-3 .login-footer a,.login-input-list .re-me,.login-input-list a').css("color", data.colourText); // rgb2hex($('.txt-var .color-block .pickr-child').css('background-color'))
      }
      ...
      $('.login-body-block input:focus + label,.login-body-block input:valid + label').css("color", data.colourText);
      ...
      $('.login-body .login-block .login-title-block span.icon-lock,.login-body .login-block .login-title-block span.login-txt,.punch .db-btn,.pass-eye').css('color', data.colourText); //rgb2hex($('.txt-var .color-block .pickr-child').css('background-color'))
      $('.sel-down-arrow').css("border-top-color", data.colourHeaderText);
      if (data.logoName == null || data.logoName == undefined || data.logoName == " " || data.logoName == "") {} else {
      ...
      if (getCookie("Remember") != undefined) {
        if (getCookie("Remember").length > 1) {
          $('.pas-code label').css({
            "top": "-10px"
          })
        }
    ...
    for (var i = 0; i < language.length; i++) {
      if (language[i].toLowerCase() == 'english') {
        ...
        langArr.push('English');
        addBannerText();
        $('.login-title').val("Welcome to Zing HR");
        $('.login-subtitle').val("Empowering employees with experiences for a better work life.");
        $('.login-link-input').val("Discover More...");
        $('.login-link-url').val("www.zinghr.com")

        $('.loginTitle').text("Welcome to Zing HR");
        $('.loginSubtitle').text("Empowering employees with experiences for a better work life.");
        $('.login-link').text("Discover More...")
        $('.login-link-url').val("www.zinghr.com");
      }
    }
    $('#themeHistory').empty().append("No Data Found !");
    if (data.PunchApplicable == "0") {
      $('.btn-group.punch ').css('display', 'none');
      $('.punch-opt .db-toggle-btn').addClass('active');
      $('.punch-opt').css('display', 'none');
    } else {
      $('.btn-group.punch ').css('display', 'flex');
      $('.punch-opt .db-toggle-btn').removeClass('active');
    }
    ...
    if (window.location.pathname.split('/').slice(-1)[0].toLowerCase() == "zing.aspx") {
      ...
      $('.main-content.login-page.setting-page').addClass('Screen-1');
    }
    $('.theme-item:nth-child(' + $('.login-page').attr('class').split(' ').pop().substr(-1) + ')').addClass('active');
    $('.login-link').attr('href', "https://www.zinghr.com");
    $('.language-select .selected-item').text('English')
    $('.cc-opt .db-toggle-btn').addClass('active');
    if (window.location.pathname.split('/').slice(-1)[0].toLowerCase() != "zing.aspx") {
      $('.esStDate').datepicker('setDate', new Date());
    }
    $('.db-overlay').css("backgroundColor", 'black');
    $('.overlay-setup .pickr-child').css("backgroundColor", 'black');
  }
  Loading('hide');
}
...
function PreviewTheme(evt) {
  ...
  if ($('.db-btn.flat-btn.sub-btn> span').attr('data-inputfieldtitle') != evt.dataset.inputfieldtitle ||
    $('.db-btn.flat-btn.sub-btn> span').attr('data-inputfieldsubtitle') != evt.dataset.inputfieldsubtitle ||
    $('.db-btn.flat-btn.sub-btn> span').attr('data-inputfieldlinktext') != evt.dataset.inputfieldlinktext ||
    $('.db-btn.flat-btn.sub-btn> span').attr('data-inputfieldlinkurl') != evt.dataset.inputfieldlinkurl ||
    $('.db-btn.flat-btn.sub-btn> span').attr('data-backgroundimagename') != evt.dataset.backgroundimagename ||
    $('.db-btn.flat-btn.sub-btn> span').attr('data-effectivestartdate') != evt.dataset.effectivestartdate ||
    $('.db-btn.flat-btn.sub-btn> span').attr('data-effectiveenddate') != evt.dataset.effectiveenddate ||
    $('.db-btn.flat-btn.sub-btn> span').attr('data-ispunchoption') != evt.dataset.ispunchoption ||
    $('.db-btn.flat-btn.sub-btn> span').attr('data-iscompanycode').toLowerCase() != evt.dataset.iscompanycode.toLowerCase()) {
    $('body').append('<div class="dp-popup previwconfirm-popup active"><div class="dp-popup-content"><div class="info-icon"><span class="icon icon-">!</span></div><div class="db-popup-head"><h2>Are you sure?</h2><h5>On preview your changes will be discarded.</h5></div><div class="db-popup-body"><div class="btn-group confirm-btn"><div class="db-btn btn-yes" onclick="previewyes()"><span>yes</span></div><div class="db-btn btn-no" onclick = "closealert()"><span>no</span></div></div></div></div></div>');
  } else {
    previewyes();
  }
...
function addBannerText() {
  for (var i = 0; i < langArr.length; i++) {
    if (langArr[i].toLowerCase() == "english") {
      if ($('.English').length < 1) {
        $('.lan-list .addonInput').append('<div class="sel-lang-block' + " " + langArr[i] + '"><div class="language-title">' + langArr[i] + '</div> <div class="bannerInput-group"><span class="banner-label">title</span> <input type="text" class="login-fields login-title" onkeyup="changeBannerText(this)" placeholder="title"></div><div class="bannerInput-group"><span class="banner-label">subtitle</span><input type="text" class="login-fields login-subtitle" onkeyup="changeBannerText(this)" placeholder="subtitle"></div><div class="bannerInput-group"><span class="banner-label">link text</span><input type="text"  class="login-fields login-link-input" onkeyup="changeBannerText(this)" placeholder="link text"></div><div class="bannerInput-group"><span class="banner-label">link url</span><input type="text" class="login-fields login-link-url" onkeyup="changeBannerText(this)" onblur="ValidURL(this.value)" placeholder="link url"></div><div class="db-error-mgs"> <span>*Please enter the required fields</span> </div><div class="db-btn input-submit sub-btn"> <span>Click to Preview</span> </div> </div>');
      }
    } else if (langArr[i].toLowerCase() == "arabic") {
      if ($('.Arabic').length < 1) {
        $('.lan-list .addonInput').append('<div class="sel-lang-block' + " " + langArr[i] + '"><div class="language-title">' + langArr[i] + '</div> <div class="bannerInput-group"><span class="banner-label">title</span><input type="text" class="login-fields login-title" id="loginTitleArabic" onkeyup="arabicValue(loginTitleArabic)" dir="rtl" placeholder="title"></div><div class="bannerInput-group"><span class="banner-label">subtitle</span><input type="text" class="login-fields login-subtitle" id="loginSubTitleArabic" onkeyup="arabicValue(loginSubTitleArabic)" dir="rtl" placeholder="subtitle"></div><div class="bannerInput-group"><span class="banner-label">link text</span><input type="text" class="login-fields login-link-input"  id="loginLinkTextArabic" onkeyup="arabicValue(loginLinkTextArabic)" dir="rtl" placeholder="link text"></div><div class="bannerInput-group"><span class="banner-label">link url</span><input type="text" class="login-fields login-link-url" id="loginLinkUrlArabic" onblur="ValidURL(this.value)" placeholder="link url"></div><div class="db-error-mgs"> <span>*Please enter the required fields</span> </div> <div class="db-btn save-arabic sub-btn" id="save-arabic"><span>Click to Preview</span> </div> </div>');
      }
    }
  }
}
...
function previewyes() {
  $('.dp-popup').fadeOut();
  Loading('show');
  $('.db-btn.flat-btn.sub-btn> span').attr('data-id')
  $('.login-title').val($('.db-btn.flat-btn.sub-btn> span').attr('data-inputfieldtitle'));
  $('.loginTitle').text($('.db-btn.flat-btn.sub-btn> span').attr('data-inputfieldtitle'));
  $('.login-subtitle').val($('.db-btn.flat-btn.sub-btn> span').attr('data-inputfieldsubtitle'));
  $('.loginSubtitle').text($('.db-btn.flat-btn.sub-btn> span').attr('data-inputfieldsubtitle'));
  $('.login-link-input').val($('.db-btn.flat-btn.sub-btn> span').attr('data-inputfieldlinktext'))
  $('.login-link-url').val($('.db-btn.flat-btn.sub-btn> span').attr('data-inputfieldlinkurl'));
  var BackgroundImageName = GetBackGroundUrl($('.db-btn.flat-btn.sub-btn> span').attr('data-backgroundimagename')); //evt.dataset.BackgroundImageName;
  $('.img-display-block img').attr("src", BackgroundImageName);
  $('.login-page').css("background-image", "url(" + BackgroundImageName + ")");

  $('.esStDate').val($('.db-btn.flat-btn.sub-btn> span').attr('data-effectivestartdate').split(' ')[0]);
  $('.esEdDate').val($('.db-btn.flat-btn.sub-btn> span').attr('data-effectiveenddate').split(' ')[0]);
  $('.imgEdDate').val($('.db-btn.flat-btn.sub-btn> span').attr('data-backgroundimageenddate').split(' ')[0]);

  if ($('.db-btn.flat-btn.sub-btn> span').attr('data-ispunchoption') == "true" || $('.db-btn.flat-btn.sub-btn> span').attr('data-ispunchoption') == true) {
    if (($('.btn-group.punch').attr('style') != undefined) && ($('.btn-group.punch').attr('style') != "")) {
      $('.punch-opt .db-toggle-btn').click();
    }
    $('.punch-opt .db-toggle-btn').removeClass('active');
  ...
  if ($('.db-btn.flat-btn.sub-btn> span').attr('data-iscompanycode').toLowerCase() == "true" || $('.db-btn.flat-btn.sub-btn> span').attr('data-iscompanycode').toLowerCase() == true) {
    if (($('.com-code').attr('style') != undefined) && ($('.com-code').attr('style') != "")) {
      $('.cc-opt .db-toggle-btn').click();
    }
    $('.cc-opt .db-toggle-btn').removeClass('active');
  } else {
  ...
  var language = $('.db-btn.flat-btn.sub-btn> span').attr('data-languageid').split(',');
  for (var i = 0; i < language.length; i++) {
    ...
    $('#loginTitleArabic').val($('.db-btn.flat-btn.sub-btn> span').attr('data-inputfieldtitle_arabic'));
    $('#loginSubTitleArabic').val($('.db-btn.flat-btn.sub-btn> span').attr('data-inputfieldsubtitle_arabic'));
    $('#loginLinkTextArabic').val($('.db-btn.flat-btn.sub-btn> span').attr('data-inputfieldlinktext_arabic'));
    $('#loginLinkUrlArabic').val($('.db-btn.flat-btn.sub-btn> span').attr('data-inputfieldlinkurl_arabic'));
  }
  $('.language-select .selected-item').text($('.db-btn.flat-btn.sub-btn> span').attr('data-languageid'));

  $('body').css("background-color", $('.db-btn.flat-btn.sub-btn> span').attr('data-colourBackground'));
  $('.db-head').css("background-color", $('.db-btn.flat-btn.sub-btn> span').attr('data-colourHeaderBackground'));
  $('.login-head .right-block .imp-link a,.login-head .right-block .imp-link a .icon').css("color", $('.db-btn.flat-btn.sub-btn> span').attr('data-colourHeaderText'));
  $('.dashboard-page h4,.loginTitle,.loginSubtitle,.login-link').css("color", $('.db-btn.flat-btn.sub-btn> span').attr('data-colourTitleText'));
  Loading('hide');
  return false;
}
